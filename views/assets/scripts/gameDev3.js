...
class Player extends Node {
  constructor(){
    ...
    scene.add(this.node);
    this.addComponent(new NetworkIdentity);
    this.networkIdentity = this.getComponent[0];
    this.username = null;
    this.scale = 0;
...
class NetworkManager {
  ...
  socketEvents(){
    ...
    socket.on("spawn", function(data) {
      ...
      var networkIdentity = player.getComponent[0];
      networkIdentity.setNetworkId(data.id, that.clientId); //network identity
      that.serverPlayers[data.id] = networkIdentity;
      if(networkIdentity.controlling){
    ...
    socket.on("disconnected", function(data) {
      ...
      that.serverPlayers[data.id].node.destroy();
